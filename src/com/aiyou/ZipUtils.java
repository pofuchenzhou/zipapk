...
public class ZipUtils {
	...
	/** 密钥 目录 */
	private static final String SECRET_KEY_PATH = System
			.getProperty("user.dir");
	/** 密钥 名称 */
	private static final String SECRET_KEY_NAME = "android.keystore";

	private static final String SECRET_KEY_PASSWORD = "123456";
	...
	public static void main(String[] args) throws Exception {
		...
		String srcPath = System.getProperty("user.dir") + "\\" + args[0];
		...
		compress(parentPath + SOURCE_PATH_NAME + prefixName, targetPath
				+ fileName);

		// ------签名
		String javaHome = System.getProperty("java.home");

		javaHome = javaHome.substring(0, javaHome.lastIndexOf("\\")) + "\\bin";
		File ff = new File(javaHome);
		...
		buffer.append("cmd.exe /c jarsigner -keystore ")
				.append(SECRET_KEY_PATH).append("\\").append(SECRET_KEY_NAME)
				.append(" -storepass ").append(SECRET_KEY_PASSWORD)
				.append(" -signedjar ").append(resultPath).append(fileName)
				.append(" ") // 签名保存路径应用名称
		...
		int rtStatus = process.waitFor();
		if (rtStatus != 0)
			System.out.println("status:" + rtStatus);
